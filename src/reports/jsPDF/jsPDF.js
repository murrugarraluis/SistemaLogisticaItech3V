...
function reportPurchase(name, columns, data, parameters, graphics) {
  const company = {
    name: 'ITECH 3V SAC',
    email: 'itech3v@gmail.com',
    address: 'Calle Ruben Guerra, Lurifico',
  ...
  doc.setFont('courier').setFont(undefined, 'normal').setFontSize(9).text(company.email, 73, 29)
  doc.setFont('helvetica').setFont(undefined, 'bold').setFontSize(14).text('REPORTE DE', 145, 17)
  doc.setFont('helvetica').setFont(undefined, 'bold').setFontSize(14).text('COMPRAS', 145, 21)
  ...
  doc.addPage()
  doc.setFont('helvetica').setFont(undefined, 'bold').setFontSize(14).text('GRAFICO', 10, 10)
  doc.addImage(graphics, 'PNG', 10, 15, 190, 130)
  window.open(URL.createObjectURL(doc.output('blob')))
}
export default {
  reportRequest,
